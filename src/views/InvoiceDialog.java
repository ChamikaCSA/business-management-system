...
public class InvoiceDialog extends JDialog {
    ...
    public InvoiceDialog(JFrame parentFrame, String title, InvoiceService invoiceService, CustomerService customerService, ItemService itemService, PaymentService paymentService, ScaleLicenseService scaleLicenseService) {
    ...
    private void saveInvoice() {
        ...
        if (customerComboBox.getSelectedIndex() == 0) {
            ...
            String customerId = Generator.generateId("CUST", customerService.getCustomerCount() + 1);
        ...
        String invoiceId = Generator.generateDatedId("INV", invoiceService.getInvoiceCount() + 1);
        Date date = new Date();
        ...
        for (Item item : items.keySet()) {
            if (item.getName().equalsIgnoreCase("scale")) {
                ScaleLicenseDialog scaleLicenseDialog = new ScaleLicenseDialog((JFrame) getParent(), "Scale License", customer, scaleLicenseService);
                scaleLicenseDialog.setVisible(true);
            }
        }
    ...
    private void sendInvoice(Invoice invoice) {
        ...
        sb.append("<html><body>");
        sb.append("<h2>Invoice Details</h2>");
        sb.append("<p><strong>Invoice ID:</strong> ").append(invoice.getId()).append("</p>");
        sb.append("<p><strong>Date:</strong> ").append(invoice.getDate()).append("</p>");
        sb.append("<p><strong>Customer:</strong> ").append(invoice.getCustomer().getName()).append("</p>");
        ...
        for (Map.Entry<Item, Integer> entry : invoice.getItemsMap().entrySet()) {
            ...
            sb.append("<tr>")
                    ...
                    .append("<td style='border: 1px solid #ddd; padding: 8px;'>$").append(String.format("%.2f", price)).append("</td>")
                    .append("</tr>");
        }

        sb.append("</table>");
        sb.append("<h3>Total Amount: $").append(String.format("%.2f", invoice.getTotalAmount())).append("</h3>");
